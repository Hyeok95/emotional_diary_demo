...
def analyze_diary(diary_text: str, model: str):
    prompt = f"""
너는 감정 전문가 AI야.
아래 일기에서 느껴지는 주요 감정을 {', '.join(EMOTION_LABELS_30)} 중 **정확히 하나**로 분류해.
그리고 아래처럼 **포맷을 꼭 맞춰서** 예쁘고 명확하게 한국어로 답변해줘.
...
[감정 이유]
(일기에서 감정이 도출된 이유 한두 문장)

[피드백]
(공감과 위로가 담긴 간단한 응원/칭찬/조언)
